<?php
/* SVN FILE: $Id: basics.php,v 1.2 2006/11/07 20:19:20 walambre Exp $ */
/**
 ...
 * @lastmodified	$Date: 2006/11/07 20:19:20 $
	...
	function loadPluginModels($plugin) {
		...
		foreach(listClasses($pluginModelDir)as $modelFileName) {
			...
			if (!class_exists($className)) {
				...
				// CTRApp custom code, EXTENDS called models (if it exists); Added by Wil Alambre, Nov 2007
				$ctrapp_file = APP . 'plugins' . DS . $plugin . DS . 'models' . DS . 'custom' . DS . $modelFileName;
				if (file_exists($ctrapp_file)) {
					require($ctrapp_file);
				}
	...
	function loadPluginController($plugin, $controller) {
		...
		if (!class_exists($controller . 'Controller')) {
			...
			if (file_exists($file)) {
				...
				// CTRApp custom code, EXTENDS called controller (if it exists); Added by Wil Alambre, Nov 2007
				$ctrapp_file = APP . 'plugins' . DS . $plugin . DS . 'controllers' . DS . 'custom' . DS . $controller . '_controller.php';
				if (file_exists($ctrapp_file)) {
					require($ctrapp_file);
				}
				
				return true;
			} else if (file_exists(APP . 'plugins' . DS . $plugin . DS . 'controllers' . DS . $plugin . '_controller.php')) {
				require(APP . 'plugins' . DS . $plugin . DS . 'controllers' . DS . $plugin . '_controller.php');
				return true;
			} else {
				return false;
