 ...
 * @access private
 */
	var $helpers = array( 'Html', 'Translations', 'othAuth', 'Pagination' );
		...
		if ( !empty($pageDetails['pageCount']) )
		{
			if(is_array($pageDetails['resultsPerPage']))
			{
				...
				foreach($pageDetails['resultsPerPage'] as $value)
				{
					...
					{
						$t .= '<span class="at">'.$value.'</span>'.$separator;
					}
					else
					{
...
**/
	...
	{
		if (empty($this->_pageDetails) || $this->_pageDetails['pageCount'] == 1) { return '<span class="at">1</span>'; }
...
**/
	...
	/*
		Added by Wil, Visual Lizard
	*/
	
	function renderPaginationElement( $paging, $colspan=1, $lang, $return=0 ) {
		...
		' ;
			...
			if ( $this->setPaging( $paging ) ) {
				...
				*/
				...
				$prev_link_text = $this->Translations->t('prev', $lang);
			    $next_link_text = $this->Translations->t('next', $lang);
				
			    $prev = $this->prevPage( $prev_link_text, false );
			    $prev = $prev ? $prev : $prev_link_text;
			    
				$next = $this->nextPage( $next_link_text, false );
			    $next = $next ? $next : $next_link_text;
				
			    $pages = $this->pageNumbers(" | ");
				
			    $display_string .= '
						<span id="pagination_results">'.$this->result( NULL ).'</span>
						<span id="pagination_links">'.$prev." ".$pages." ".$next.'</span>
