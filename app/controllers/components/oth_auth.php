...
*/
    ...
    var $controller    = true;
    var $gid = 1;
    var $redirect_page = '/menus';
    var $hashkey       = "CTRAppHashKey";
    var $auto_redirect = true;
    
    var $login_page    = '/';
    var $logout_page   = '/users/logout';
    var $access_page   = '/menus';
    var $noaccess_page = '/users/noaccess'; // session_flash, flash, back or a page url
    ...
    var $allowedAssocBankModels      = array();
   ...
   function _login($params,$ignore_cookie = false)
   {
    ...
    function _addLog( &$row ) {
        
        if ( $this->log_active ) {
        
			$data[$this->log_model]['user_id']  = $row[$this->user_model]['id'];
			$data[$this->log_model]['url']  = $this->controller->here;
			$data[$this->log_model]['visited'] = date('Y-m-d h:i:s');
			$data[$this->log_model]['allowed'] = $this->_othCheckPermission($row) ? '1' : '0';
			
			if (!class_exists($this->log_model)) {        
				loadModel($this->log_model);
			}
			
			$LogModel =& new $this->log_model;
			$LogModel->save($data);
    ...
    function _othCheckPermission( &$ses, $multi=false, $check_this_link=NULL )
    {
        //die('c');
        uses('inflector');
        
        // use $check_this_link variable, because of CheckMenuPermissions functionality...
        if ( !isset($check_this_link->name) ) {
        	$check_this_link = $this->controller;
        }
        
        
        // echo ' - '.$check_this_link->plugin.'/'.$check_this_link->name.'/'.$check_this_link->action;
        
        
       	// tack on PLUGIN name for checking permissons
		$c   = strtolower(Inflector::underscore($check_this_link->name));
		if ( isset($check_this_link->params['plugin']) ) { $c = strtolower($check_this_link->params['plugin']).( $c ? '/'.$c : '' ); }
		
		$a   = strtolower($check_this_link->action);
        ...
        $params = isset($check_this_link->params['pass'])?implode('/',$check_this_link->params['pass']): '';
		...
		if(is_string($check_this_link->othAuthRestrictions))
        {
            
            $is_checkall   = $check_this_link->othAuthRestrictions === "*";
            $is_cake_admin = defined('CAKE_ADMIN') && ($check_this_link->othAuthRestrictions === CAKE_ADMIN);
        ...
        else if ( is_array($check_this_link->othAuthRestrictions) )
        {
            
            $a_p_in_array = in_array($a.'/'.$params, $check_this_link->othAuthRestrictions);
   ...
   function _aclCheckAccess($aro_alias, $aco)
   {
    ...
    function _createModel()
    {
        ...
        */
        ...
        $forUser  = array(
        	'belongsTo'=>array($this->group_model),
        	'hasMany'=>array(),
        	'hasAndBelongsToMany'=>array()
		);
		
        $forGroup = array(
        	'belongsTo'=>array($this->bank_model),
        	'hasMany'=>array(),
        ...
        // added by Wil for CTRApp, Dec 7 2007
        $forBank = array(
        	'belongsTo'=>array(),
        	'hasMany'=>array(),
        	'hasAndBelongsToMany'=>array()
        ...
        // added by Wil for CTRApp, Dec 7 2007
        $forBank  = $this->_mergeModelsToKeep($forBank,$this->allowedAssocBankModels);
        ...
        // added by Wil for CTRApp, Dec 7 2007
        $UserModel->{$this->group_model}->{$this->bank_model}->unbindAll($forBank);
   ...
   function url($url = null)
   {
	...
	/*
		Wil@VisualLizard added this modification... 
		Check permissions against passed plugin/controller/action set 
		Used in MENUS controllers 
	*/
    
    function checkMenuPermission( $check_this_link=NULL ) {
        ...
        if( $this->Session->valid() && $this->Session->check($us) )  {
			...
			if ( $check_this_link ) {
				...
				$use_this_to_check = NULL;
				
				// $check_this_link should be "/PLUGIN/CONTROLLER/ACTION/PARAMS"
				$check_this_link = explode('/', $check_this_link);
				
				$use_this_to_check->params['plugin'] = isset($check_this_link[1]) ? $check_this_link[1] : '';
				$use_this_to_check->name = isset($check_this_link[2]) ? $check_this_link[2] : '';
				$use_this_to_check->action = isset($check_this_link[3]) ? $check_this_link[3] : '';
				$use_this_to_check->params['pass'] = array();
				$use_this_to_check->othAuthRestrictions = $this->controller->othAuthRestrictions;
				
				// check permissions against provided link
				$return = $this->_othCheckPermission( $ses, false, $use_this_to_check );
